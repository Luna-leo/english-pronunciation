...
└─ .sheet（図枠: 二重枠+トンボ・幅 var(--frame)・内部は無地 --paper・flex column・全幅で画面高に固定）
   └─ .sheet-body … flex:1・min-height:0・overflow-y:auto — ここがスクロール領域（枠・トンボは固定、中身だけ流れる）
      ├─ header.sheet-head … .wordmark（等幅 --h1・字間.14em。中身は `Link href="/"` = ホームへの動線。hover/focus は朱の細下線のみ）+ .subtitle（字間.32em）+ .ruler — スクロールで流れる
      └─ {ページ} … Crumbs（sticky top-0・貼り付くとコンテキストバー化）+ 本文
```

### 高さ方針

- **全幅で固定（PC/タブレット/モバイル同一）**: 図枠を画面高（100dvh）に固定し、コンテンツは `.sheet-body` の内側でスクロール（枠・トンボは常に固定）。ヘッダー `.sheet-head` は `.sheet-body` の**内側**に置かれ、本文と一緒にスクロールで流れる（Issue #9: PC でヘッダーが縦を浪費／モバイルで流れる、という挙動の逆転を解消）
- `.sheet-body` の `min-height:0` を消すと内部スクロールが効かなくなる（flexの定石）
- **（旧）固定解除フォールバックは撤去**: 以前は `@media (max-width:1020px)` で `height:auto; max-height:none; overflow:visible` にしてページ全体スクロールへ切り替えていたが廃止。全幅で枠固定＋内部スクロールに統一した（過去の「`max-height` 解除忘れで枠外へあふれる」既知バグは、固定解除自体をやめたため発生経路が消滅）
- **Safari の sticky 再描画バグ（要注意・継続）**: `body`（sticky な `Crumbs` の祖先）に `overflow-x: clip`/`hidden` を**付けない**こと。付けると Safari でスクロールで通り過ぎたコンテンツが画面最上部に残像として焼き付く（Chrome では出ない／Safari 限定）。横はみ出し対策は `overflow` ではなくワードマーク縮小（下記）で行う
- **iOS 26 Safari のステータスバー透け（解消済み）**: 旧実装はスマホを全体スクロールにしていたため、半透明グラス(Liquid Glass)のステータスバー裏に本文が透けた（iOS 26 は `theme-color` を無視し Web から不透明化不可・Apple Dev Forums でも確認済み）。本実装で全幅「枠固定＋内部スクロール」に統一したため**透けは発生しない**（＝かつて「今回は不採用」と記していた本来の解を採用）。なお関連方針は維持: (1) `html`/`body` の `background-color` を `--paper` の無地に保つ、(2) **sticky/fixed なバー(例 `Crumbs`)は背景色・影・`backdrop-filter` を要素自体に置かず絶対配置の子に逃がす**（iOS 26 が sticky 要素の背景を拾って誤色付けするのを防ぐ）、(3) `themeColor:"#f6f4ee"` は Android Chrome の上端バー用に残置（リテラル hex は `:root` の `--paper` と同値に保つ）
...
`components/ui/Crumbs.tsx`（client component）。`.sheet-body` 内をスクロールすると上端に貼り付く GitHub 風バー。

- **貼り付き検知**: バー直前に高さ0のセンチネルを置き、`IntersectionObserver`（`root` = 祖先の `.sheet-body`。無ければ viewport にフォールバック）でセンチネルが上端を抜けた＝ `stuck` を判定。`stuck` は `data-stuck` 属性としてルートに付与。
- **未スクロール時**: 装飾なし（背景・罫・影を透明に）。パンくず＋VOICE バッジのみ表示。
- **貼り付き時**: 絶対配置の子に置いた背景（`--paper`）＋下罫＋控えめな影を `opacity` で出現させ、`title`（音素名 例 `/ɪ/ kit` — 詳細ページのみ渡す）を `translate`＋`opacity` でスライドイン。直下の大見出しとの重複を避けるため、音素名はスクロール後のみ表示する（未スクロール時も領域は確保しレイアウトシフトを防ぐ）。
- 背景・影は **要素自体でなく絶対配置の子**に置く（上記 iOS 26 対策と同手法）。アニメは Tailwind `transition`＋`group-data-[stuck]` で、`prefers-reduced-motion` 時は §5 のグローバル規則で即時切替になる。

### 原典からの意図的な逸脱

1. ポータル header の `border-bottom:1.5px` は省略（lede/meta の無い簡素ヘッダーではルーラーと罫が二重になるため）
2. シートは全幅で `height:100%`/`max-height:100%` により常に画面高を満たす（`body` は `align-items:center`。原典は flex-start + 自然高）
...
- サブページの `<main>` 先頭は `Crumbs`（`components/ui/Crumbs.tsx`）。`sticky top-0` で `.sheet-body` のスクロール時に上端へ貼り付き、貼り付いた時だけ `bg-paper` の帯 + 下罫 `border-ink-faint` + 控えめな影が出てコンテキストバー化する（未スクロール時は装飾なし）。詳細ページは `title` に音素名を渡すと貼り付き時にスライドイン表示される。リンクは `text-ink-soft` + 下線・現在地は下線なし、hover で朱（詳細は §6 コンテキストバー）
